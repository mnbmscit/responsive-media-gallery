<?php
/*
Plugin Name: Responsive Media Gallery
Description: Replace the default Wordpress gallery with a responsive and touch friendly Galleria based slideshow.
Version: 1.0
Author: DigitalCourt
Author URI: http://DigitalSafari.co
License: GPL

Fork of Image Gallery Reloaded
http://wordpress.org/plugins/image-gallery-reloaded/
*/
...
{	
	...
	if(!is_admin()){
		...
		wp_enqueue_script ('gallery_reloaded_source', plugins_url('/min/galleria.min.js', __FILE__), array('jquery'));
		wp_register_style ( 'gallery_style', plugins_url('/themes/' . $theme . '/galleria.classic.css', __FILE__) );
...
function igr_gallery_shortcode($attr)
{
	...
	$output .= "<script type='text/javascript'>Galleria.loadTheme('" . plugins_url('/responsive-media-gallery/themes/'. $theme . '/galleria.classic.min.js') . "');</script>";
